...
const stylish = (arrOfObjs) => {
  const iter = (objs, depth = 0) => {
    ...
    return objs.flatMap((obj) => {
      switch (obj.status) {
        case 'added':
          return [`${nextSpace}+ ${obj.key}: ${stringify(obj.value, depth + 1)}`]
        case 'deleted':
          return [`${nextSpace}- ${obj.key}: ${stringify(obj.value, depth + 1)}`]
        case 'changed':
          return [`${nextSpace}- ${obj.key}: ${stringify(obj.value1, depth + 1)}`, `${nextSpace}+ ${obj.key}: ${stringify(obj.value2, depth + 1)}`]
        case 'unchanged':
          return [`${nextSpace}  ${obj.key}: ${stringify(obj.value, depth + 1)}`]
        case 'recursion':
          return [`${nextSpace}  ${obj.key}: {`, ...iter (obj.children, depth + 1), `${nextSpace}  }`]
        default:
          throw new Error('Unknown AST status')
      }
    })
  }
